...
export function QueueDisplay({ queue, isDarkMode, onRetry }: QueueDisplayProps) {
  ...
  const getStatusIcon = (status: string) => {
    if (status.includes('Downloading')) {
      return <Download style={{ width: '1.25rem', height: '1.25rem', color: isDarkMode ? '#60a5fa' : '#3b82f6' }} />
    } else if (status.includes('Selesai') || status.includes('Completed')) {
      return <CheckCircle style={{ width: '1.25rem', height: '1.25rem', color: isDarkMode ? '#4ade80' : '#22c55e' }} />
    } else if (status.startsWith('Error')) {
      return <AlertCircle style={{ width: '1.25rem', height: '1.25rem', color: isDarkMode ? '#f87171' : '#ef4444' }} />
    } else {
      return <XCircle style={{ width: '1.25rem', height: '1.25rem', color: isDarkMode ? '#9ca3af' : '#6b7280' }} />
    }
  }
